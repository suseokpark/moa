...
test("restore checkpoint survives edits, folds and undo, then recovery can itself be undone", async () => {
  ...
  const undoFold = await handle({ type: "FAVMOA_UNDO", expectedRevision: 3 }, sender);
  assert.equal(undoFold.hasRestorePoint, true);
  ...
  assert.deepEqual(undoRecovery.catalog, edited.catalog);
